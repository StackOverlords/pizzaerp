...
const TEST = {
  tenantSlug: 'test-dough-wastages-tenant',
...
beforeAll(async () => {
  ...
  const plan = await prisma.plan.upsert({
    where: { name: '_test-plan-dough-wastages' },
    update: {},
    create: { name: '_test-plan-dough-wastages' },
  })
...
afterAll(async () => {
  ...
  await prisma.plan.deleteMany({ where: { name: '_test-plan-dough-wastages' } })
...
// ─── POST /api/v1/dough-wastages ──────────────────────────────────────────────

describe('POST /api/v1/dough-wastages', () => {
  it('DW-01 — ADMIN registra merma correctamente', async () => {
    const res = await server.inject({
      method: 'POST',
      url: '/api/v1/dough-wastages',
      headers: authHeader(adminToken),
      payload: { doughType: 'SMALL', quantity: 3, reason: 'FELL' },
    })
    expect(res.statusCode).toBe(201)
    const body = res.json()
    expect(body.id).toBeDefined()
    expect(body.doughType).toBe('SMALL')
  ...
  it('DW-02 — CAJERO también puede registrar merma', async () => {
    const res = await server.inject({
      ...
      payload: { doughType: 'MEDIUM', quantity: 1, reason: 'BAD_SHAPE' },
    })
    expect(res.statusCode).toBe(201)
  ...
  it('DW-03 — motivo OTHER requiere nota (400 sin nota)', async () => {
    const res = await server.inject({
      ...
      url: '/api/v1/dough-wastages',
      headers: authHeader(adminToken),
      payload: { doughType: 'LARGE', quantity: 2, reason: 'OTHER' },
    })
  ...
  it('DW-04 — motivo OTHER con nota registra correctamente', async () => {
    const res = await server.inject({
      ...
      url: '/api/v1/dough-wastages',
      headers: authHeader(adminToken),
      payload: { doughType: 'LARGE', quantity: 2, reason: 'OTHER', notes: 'Se dañaron en el horno' },
    })
    expect(res.statusCode).toBe(201)
  ...
  it('DW-05 — cantidad 0 devuelve 400', async () => {
    const res = await server.inject({
      ...
      url: '/api/v1/dough-wastages',
      headers: authHeader(adminToken),
      payload: { doughType: 'SMALL', quantity: 0, reason: 'FELL' },
    })
    expect(res.statusCode).toBe(400)
  })
...
// ─── GET /api/v1/dough-wastages ───────────────────────────────────────────────

describe('GET /api/v1/dough-wastages', () => {
